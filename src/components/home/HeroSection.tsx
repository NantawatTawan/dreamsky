    ...
    <section className="relative min-h-[88vh] md:min-h-screen overflow-hidden flex items-center">
      <div
        ...
        style={{
          ...
          background:
            'radial-gradient(120% 80% at 50% 20%, #2a4a78 0%, #1B3A5C 55%, #0f2340 100%)',
        }}
      />
      <div
        aria-hidden
      ...
      <div
        aria-hidden
        className="absolute inset-0 -z-10 bg-gradient-to-b from-transparent via-transparent to-navy/70"
      />
      ...
      <div className="mx-auto max-w-6xl w-full px-5 md:px-8 py-20 md:py-28">
        ...
        >
          <Plane className="inline-block h-3.5 w-3.5 mr-1 -mt-0.5" /> {t('note')}
        </span>
        ...
        <h1 className="mt-6 font-extrabold text-white leading-[1.05] tracking-tight text-4xl sm:text-5xl md:text-7xl">
          {lines.map((line, i) => (
            <span
              ...
              style={{ animationDelay: `${200 + i * 300}ms`, animationFillMode: 'forwards' }}
            >
              {i === 1 ? (
      ...
      >
        <span className="text-xs md:text-sm font-medium">{t('scroll_hint')}</span>
        <svg width="20" height="28" viewBox="0 0 20 28" fill="none">
          <path d="M10 2v22M2 18l8 8 8-8" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
      </button>
    </section>
  );
